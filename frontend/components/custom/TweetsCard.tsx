...
import { HeartIcon, MoreVertical, Redo2 } from "lucide-react";
...
const TweetsCard: React.FC<TweetsCardProps> = ({
  ...
  const loadMoreTweets = useCallback(async () => {
    const nextPage = (page % 7) + 1; // Cycle through pages 1-7
  ...
  }, [inView, loadMoreTweets]); // loadMoreTweets is added to the dependency array
        ...
        <div className="ml-auto flex items-center gap-1">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => exportToCSV(tweets, "Tweets")}>CSV</DropdownMenuItem>
              <DropdownMenuItem onClick={() => exportToJSON(tweets, "Tweets")}>JSON</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </CardHeader>
              ...
              <div key={tweet.id} className="flex flex-col gap-2 py-1">
                ...
                <div className="flex flex-row gap-2 items-center">
                  <div className="flex flex-row gap-1 items-center">
                    ...
                    <span className="font-medium leading-10">{tweet.favorite_count}</span>
                  </div>
                  <div className="flex flex-row gap-1 items-center">
                    <Redo2 className="h-5 w-5 " />
                    <span className="font-medium leading-10">{tweet.retweet_count}</span>
                  </div>
                  ...
                  <Separator className="w-[1px] bg-primary h-4" />
                  <p className="font-regular">{new Date(tweet.created_at).toLocaleString()}</p>
                </div>
                {tweet.symbols.length > 0 && (
                  <div className="flex items-start flex-col">
                    <span className="font-medium leading-10">Tweet Hashtags:</span>
                    <div className="flex flex-row flex-wrap gap-1 items-center">
                      {tweet.symbols.map((symbol) => (
                        <Link
                          href={`https://twitter.com/search?q=%23${symbol.symbol.slice(1, symbol.symbol.length)}`}
                          key={symbol.id}
                          className="font-medium text-blue-500"
                        >
                          {symbol.symbol}
