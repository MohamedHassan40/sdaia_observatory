...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  const tweetData = await fetch(
    ...
    `http://localhost:3000/api/getTweetsForCompany?id=${params.id}`, {
  ...
  const newsData = await fetch(
    ...
    `http://localhost:3000/api/getNewsForCompany?id=${params.id}`, {
  ...
  const blogsData = await fetch(
    ...
    `http://localhost:3000/api/getBlogsForCompany?id=${params.id}`, {
  ...
  const productsData = await fetch(
    ...
    `http://localhost:3000/api/getProductsForCompany?id=${params.id}`, {
